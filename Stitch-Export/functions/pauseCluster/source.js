exports = async function(projectId, clusterName){
  ...
  if((projectId!= "5b6c7a1496e82139bc824e8b") && (clusterName != "Example")) {
    ...
    conn.findOne({"name":clusterName, "projectId":projectId}).then((srcdoc) => {
      if (srcdoc) {
        var protect = (srcdoc.protected || false);
        if(!protect) {
          ...
          var result = httpService.patch(args, {"encodeBodyAsJSON":true}).then((res) => {
          ...
          console.log("NEVER REAP " + clusterName);
        }
      } 
    });
